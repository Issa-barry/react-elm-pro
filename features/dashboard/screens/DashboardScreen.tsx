...
const PERIOD_CONFIG: Record<Period, { factor: number; changes: ChangeMap }> = {
  aujourdhui: {
    factor: 1 / 30,
    changes: {
      factures:     { pct: '+3%',  trend: 'up'   },
      payees:       { pct: '+5%',  trend: 'up'   },
      encaissement: { pct: '-1%',  trend: 'down' },
      ventes:       { pct: '+3%',  trend: 'up'   },
      clients:      { pct: '+0%',  trend: 'up'   },
      produits:     { pct: '+4%',  trend: 'up'   },
    },
  },
  hier: {
    factor: 1 / 32,
    changes: {
      factures:     { pct: '+1%',  trend: 'up'   },
      payees:       { pct: '+2%',  trend: 'up'   },
      encaissement: { pct: '-2%',  trend: 'down' },
      ventes:       { pct: '+1%',  trend: 'up'   },
      clients:      { pct: '+0%',  trend: 'up'   },
      produits:     { pct: '+2%',  trend: 'up'   },
    },
  },
  cette_semaine: {
    factor: 1 / 4.3,
    changes: {
      factures:     { pct: '+6%',  trend: 'up'   },
      payees:       { pct: '+8%',  trend: 'up'   },
      encaissement: { pct: '-2%',  trend: 'down' },
      ventes:       { pct: '+7%',  trend: 'up'   },
      clients:      { pct: '+1%',  trend: 'up'   },
      produits:     { pct: '+9%',  trend: 'up'   },
    },
  },
  semaine_derniere: {
    factor: 1 / 4.8,
    changes: {
      factures:     { pct: '+4%',  trend: 'up'   },
      payees:       { pct: '+6%',  trend: 'up'   },
      encaissement: { pct: '-3%',  trend: 'down' },
      ventes:       { pct: '+5%',  trend: 'up'   },
      clients:      { pct: '+0%',  trend: 'up'   },
      produits:     { pct: '+7%',  trend: 'up'   },
    },
  },
  ce_mois: {
    factor: 1,
    changes: {
      factures:     { pct: '+8%',  trend: 'up'   },
      payees:       { pct: '+15%', trend: 'up'   },
      encaissement: { pct: '-3%',  trend: 'down' },
      ventes:       { pct: '+12%', trend: 'up'   },
      clients:      { pct: '+2%',  trend: 'up'   },
      produits:     { pct: '+22%', trend: 'up'   },
    },
  },
  mois_dernier: {
    factor: 0.88,
    changes: {
      ...
      payees:       { pct: '+5%',  trend: 'up'   },
      encaissement: { pct: '-1%',  trend: 'down' },
      ventes:       { pct: '+4%',  trend: 'up'   },
      clients:      { pct: '+1%',  trend: 'up'   },
      produits:     { pct: '+8%',  trend: 'up'   },
    },
  },
  ce_trimestre: {
    factor: 3.1,
    changes: {
      factures:     { pct: '+11%', trend: 'up'   },
      payees:       { pct: '+18%', trend: 'up'   },
      encaissement: { pct: '-6%',  trend: 'down' },
      ventes:       { pct: '+15%', trend: 'up'   },
      clients:      { pct: '+8%',  trend: 'up'   },
      produits:     { pct: '+25%', trend: 'up'   },
    },
  },
  trimestre_dernier: {
    factor: 2.75,
    changes: {
      factures:     { pct: '+7%',  trend: 'up'   },
      payees:       { pct: '+12%', trend: 'up'   },
      encaissement: { pct: '-4%',  trend: 'down' },
      ventes:       { pct: '+9%',  trend: 'up'   },
      clients:      { pct: '+5%',  trend: 'up'   },
      produits:     { pct: '+18%', trend: 'up'   },
    },
  },
  cette_annee: {
    factor: 12.5,
    changes: {
      factures:     { pct: '+14%', trend: 'up'   },
      payees:       { pct: '+22%', trend: 'up'   },
      encaissement: { pct: '-8%',  trend: 'down' },
      ventes:       { pct: '+18%', trend: 'up'   },
      clients:      { pct: '+12%', trend: 'up'   },
      produits:     { pct: '+30%', trend: 'up'   },
    },
  },
  annee_derniere: {
    factor: 11,
    changes: {
      factures:     { pct: '+9%',  trend: 'up'   },
      payees:       { pct: '+15%', trend: 'up'   },
      encaissement: { pct: '-5%',  trend: 'down' },
      ventes:       { pct: '+12%', trend: 'up'   },
      clients:      { pct: '+8%',  trend: 'up'   },
      produits:     { pct: '+22%', trend: 'up'   },
    },
...
function buildKpis(period: Period): KpiData[] {
  ...
  return (Object.keys(B) as (keyof typeof B)[]).map(id => {
    ...
    return {
      ...
      label:  kpiLabel(id),
      value,
      change: changes[id].pct,
      trend:  changes[id].trend,
      points: scalePoints(BASE_POINTS[id], factor),
    };
  });
}
...
function buildSvgPoints(raw: number[]): string {
  ...
  const step  = SPARK_W / (raw.length - 1);
  const pad   = 3;
  return raw
    .map((v, i) => {
...
// ─── Infrastructure SVG générique (donut + camembert) ────────────────────────
...
const SEG_GAP   = 2.5; // degrés de séparation entre segments

// Taille grande (carte pleine largeur)
const LG_SIZE   = 180;
const LG_CX     = LG_SIZE / 2;
const LG_CY     = LG_SIZE / 2;
const LG_RO     = 78;
const LG_RI     = 50; // donut

// Taille petite (grille 2 colonnes)
const SM_SIZE   = 130;
const SM_CX     = SM_SIZE / 2;
const SM_CY     = SM_SIZE / 2;
const SM_RO     = 56;
const SM_RI_DON = 35; // donut
const SM_RI_PIE = 0;  // camembert
...
// ri <= 0 → tranche camembert depuis le centre ; ri > 0 → tranche donut
function arcPath(cx: number, cy: number, ro: number, ri: number, a1: number, a2: number): string {
  const s  = polarXY(cx, cy, ro, a1);
  const e  = polarXY(cx, cy, ro, a2);
  ...
  const i1 = polarXY(cx, cy, ri, a1);
  const i2 = polarXY(cx, cy, ri, a2);
  return [
...
function buildArcs(
  segs: ChartSeg[], cx: number, cy: number, ro: number, ri: number
...
interface ChartProps {
  ...
  ri:          number;
  valueBase:   number;
  formatValue: (base: number, pct: number) => string;
}
  ...
  return (
    <>
      <Svg width={size} height={size}>
        {arcs.map((arc, i) => (
          <Path key={segs[i].id} d={arc.path} fill={arc.color} />
        ))}
...
// ─── Catégorie de véhicule (donut, pleine largeur) ────────────────────────────
  ...
  return (
    <View style={styles.chartCard}>
      ...
      <View style={styles.chartBody}>
        <ChartWidget
          segs={VEHICLE_SEGS}
          cx={LG_CX} cy={LG_CY} size={LG_SIZE} ro={LG_RO} ri={LG_RI}
          valueBase={facturesTotal}
          formatValue={(b, p) => fmtGNF(b * p)}
        />
      </View>
    </View>
  );
...
// ─── CA par site (donut) + Ventes par produit (camembert) — grille 2 col ──────
  ...
  return (
    <View style={styles.chartCard}>
      ...
      <View style={styles.chartBody}>
        <ChartWidget
          segs={PRODUIT_SEGS}
          cx={LG_CX} cy={LG_CY} size={LG_SIZE} ro={LG_RO} ri={0}
          valueBase={ventesTotal}
          formatValue={(b, p) => `${fmtN(b * p)} vte${Math.round(b * p) > 1 ? 's' : ''}`}
        />
      </View>
    </View>
  );
...
function PeriodPicker({
  value,
  onChange,
}: Readonly<{ value: Period; onChange: (p: Period) => void }>) {
  const { colors } = useTheme();
  const styles     = useMemo(() => makeStyles(colors), [colors]);
            ...
            {PERIODS.map(p => {
              ...
              return (
                <TouchableOpacity
                  ...
                  onPress={() => { onChange(p.key); setOpen(false); }}
                  activeOpacity={0.6}>
                  <Text style={[styles.sheetRowText, selected && { color: colors.primary, fontWeight: '600' }]}>
                    {p.label}
                  </Text>
                  {selected && (
